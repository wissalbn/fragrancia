...
echo "<a href='add_produit.php' class='btn btn-primary'>Ajouter un prduit</a>";
...
if ($resultat_produit && mysqli_num_rows($resultat_produit) > 0) {
    ...
    while ($row_produit = mysqli_fetch_assoc($resultat_produit)) {
        ...
        echo "<td><a href='edit_produit.php?id=" . $row_produit["IDPROD"] . "' class='btn btn-primary'>Éditer</a> ";
        echo "<a href='delete_produit.php?id=" . $row_produit["IDPROD"] . "' class='btn btn-danger'>Supprimer</a></td></tr>";
    }
    echo "</tbody>";
    echo "</table>";
} else {
